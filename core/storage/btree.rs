...
impl BTreeCursor {
    ...
    fn process_overflow_read(
        ...
        let res = match &mut self.state {
            ...
            CursorState::Read(ReadPayloadOverflow::ProcessPage {
                ...
                let next = contents.overflow_next_pointer();
            ...
            WriteState::BalanceNonRootWaitLoadPages => {
                ...
                */
                ...
                /* do not take last page */
                {
                    ...
                    assert!(
                        left_pointer <= *self.pager.db_header.database_size.lock(),
    ...
    fn balance_root(&mut self) {
        ...
        // 2. Modify root
        root_contents.leaf_to_interior(child.get().id as u32, self.usable_space() as u16);
        ...
        loop {
            ...
            match state {
                ...
                OverflowState::ProcessPage { next_page } => {
                    if next_page < 2
                        || next_page as usize > *self.pager.db_header.database_size.lock() as usize
                    {
...
fn find_free_cell(page_ref: &mut PageContent, usable_space: u16, amount: usize) -> Result<usize> {
    ...
    while pc <= maxpc {
        ...
        if amount <= size as usize {
            ...
            if new_size < 4 {
                ...
                page_ref.update_fragmented_bytes_count(frag);
                return Ok(pc);
...
pub fn btree_init_page(page: &PageRef, page_type: PageType, offset: usize, usable_space: u16) {
    ...
    contents.init(offset, page_type, usable_space);
}
...
fn edit_page(
    ...
    debug_validate_cells!(page, usable_space);
    // TODO: noverflow
    page.update_cell_count(number_new_cells as u16);
    Ok(())
}
...
fn page_free_array(
    ...
    for i in first..first + count {
        ...
        if cell_pointer.start >= buf_range.start && cell_pointer.start < buf_range.end {
            ...
            page.update_cell_count(page.cell_count() as u16 - 1);
...
fn free_cell_range(
    ...
    let pc = if page.first_freeblock() == 0 {
        ...
        page.update_fragmented_bytes_count(frag);
        pc
    };
    ...
    if offset <= page.cell_content_area() {
        ...
        page.update_first_freeblock(pc);
        page.update_content_area(end);
    } else {
...
fn defragment_page(page: &mut PageContent, usable_space: u16) {
    debug_validate_cells!(page, usable_space);
...
fn insert_into_cell(
    ...
    page.update_cell_count(new_n_cells);
    debug_validate_cells!(page, usable_space);
    Ok(())
}
...
fn allocate_cell_space(page_ref: &mut PageContent, amount: u16, usable_space: u16) -> Result<u16> {
    ...
    if gap + 2 + amount > top {
        ...
        top = page_ref.cell_content_area() as usize;
    }

    top -= amount;

    page_ref.update_content_area(top as u16);
...
fn drop_cell(page: &mut PageContent, cell_idx: usize, usable_space: u16) -> Result<()> {
    ...
    if page.cell_count() > 1 {
        ...
        page.update_content_area(usable_space);
        page.update_first_freeblock(0);
        page.update_fragmented_bytes_count(0);
    }
    page.update_cell_count(page.cell_count() as u16 - 1);
...
mod tests {
    ...
    fn empty_btree() -> (Rc<Pager>, usize) {
        ...
        let pager = {
            let db_header = Arc::new(db_header.clone());
    ...
    #[allow(clippy::arc_with_non_send_sync)]
    fn setup_test_env(database_size: u32) -> (Rc<Pager>, Arc<DatabaseHeader>) {
        let page_size = 512;
        let mut db_header = DatabaseHeader::default();
        db_header.page_size = page_size;
        db_header.database_size = Rc::new(SpinLock::new(database_size));
        let db_header = Arc::new(db_header);
        ...
        let buf = Arc::new(RefCell::new(Buffer::allocate(page_size as usize, drop_fn)));
        {
            ...
            sqlite3_ondisk::write_header_to_buf(buf_slice, &db_header);
        }
    ...
    pub fn test_clear_overflow_pages() -> Result<()> {
        ...
        while current_page <= 4 {
...

            {
                ...
                contents.update_overflow_next_pointer(next_page); // Write pointer to next overflow page
        ...
        let initial_freelist_pages = *db_header.freelist_pages.lock();
    ...
    pub fn test_clear_overflow_pages_no_overflow() -> Result<()> {
        ...
        match clear_result {
            CursorResult::Ok(_) => {
                ...
                assert_eq!(
                    *db_header.freelist_trunk_page.lock(),
                    0,
    ...
    fn test_btree_destroy() -> Result<()> {
        ...
        // Configure the root page to point to the two leaf pages
        {
            ...
            contents.update_rightmost_pointer(page4.get().id as u32);
        ...
        let initial_free_pages = *db_header.freelist_pages.lock();
        assert_eq!(initial_free_pages, 0, "should start with no free pages");

        run_until_done(|| cursor.btree_destroy(), pager.deref())?;

        let pages_freed = *db_header.freelist_pages.lock() - initial_free_pages;
